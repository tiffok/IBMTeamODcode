...
def predplot(df, name, dname):
    '''
    creates interactive plot of PCA-transformed data
    color = Outlier prediction (outlier/inlier) 
    '''
    ...
    p.scatter(
        ...
        color = factor_cmap('predout', palette=rpal, factors=cfactor),
        legend_group = 'predout')
    p.xaxis.axis_label = "Principal Component 1"
    p.yaxis.axis_label = "Principal Component 2"
    output_file("{}_outlierplot.html".format(dname))
    show(p)

def voteplot(df, name, dname):
    '''
    creates interactive plot of PCA-transformed data.
    color = total number or outlier votes
    shape = outlier prediction (outlier/inlier)
    '''
    source = ColumnDataSource(df)
    ...
    opal = ('#787878','#C1A6A6','#ffca3a','#46B1A8','#500EB3','#FF0000')
    ...
    output_file("{}_voteplot.html".format(dname))
    show(p)
...
def modelEvals(a_out, y_pred, y_score, df, name):
    ...
    (confustion matrix, roc curve, 
    number of true/false positives/negatives, AUC, etc.)
    '''
...
def createData(str):
    '''
    load in MAT file and return:
    x_pca: pca transformed data 
    a_out: actual outliers from loaded dataset [for datasets with established truth]
    data: original data in dataframe format
    '''
    #read in data
    #read in mat file
...
def runModel(x, method, name, dname, a_out):
    ...
    parameters:
    x: dataset
    method: outlier detection method to run
    name: name of method (as string)
    dname: name of dataset
    a_out: actual outliers

    runs a given method, prints model evaluations for that method
    and returns a dataframe 
    of the predictions, anomaly scores, and 
    outlier classification (correctly/incorrectly classified)
    '''
...
def getPreds(model, X):
    '''
    returns the outlier prediction (0 for inlier, 1 for outlier)
...
def validate(x):
    '''
    checks if inputted entry is either Y or N
    '''
    done = False
    while not done:
        try:
