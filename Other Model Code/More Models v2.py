...
clf = KNN(n_neighbors=20, method='largest')
...
clf = KNN(n_neighbors=20, method='median')
...
predplot(y_pred6)
actualplot(a_out)
modelEvals(a_out,y_pred6, y_scores6)
...
#fpr7, tpr7, threshold7 = roc_curve(a_out, proba7[:,0], y_pred7)
#auc7 = roc_auc_score(a_out, y_scores7)

plt.plot(fpr1, tpr1, color = "darkorange", label = f"KNN Largest Method Model: AUC = {auc1:.3f}")
plt.plot(fpr2, tpr2, color = "red", label = f"KNN Mean Model: AUC = {auc2:.3f} ")
plt.plot(fpr3, tpr3, color = "green", label = f"KNN Median Model: AUC = {auc3:.3f}")
plt.plot(fpr4, tpr4, color = "lightblue", label = f"OCSVM Model: AUC = {auc4:.3f}")
plt.plot(fpr5, tpr5, color = "purple", label = f"MCD Model: AUC = {auc5:.3f}")
plt.plot(fpr6, tpr6, color = "deeppink", label = f"ROD Model: AUC = {auc6:.3f}")
